...
def merge_cart(sender, request, user, **kwargs):
    ...
    for game_id in session_cart:
        ...
        game = Game.objects.get(id=game_id) if model == 'game' else DLC.objects.get(id=game_id)
        ...
        if game in [i.game or i.dlc for i in cart_items]:
            ...
            if game.model_name() == 'game':
                db_cart[0].cartitems.create(game=game, quantity=quantity, price=game.final_price)
            else:
                db_cart[0].cartitems.create(dlc=game, quantity=quantity, price=game.final_price)